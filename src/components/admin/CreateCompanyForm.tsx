...
export default function CreateCompanyForm({
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const res = await axios.get('https://pplus-6xcn.onrender.com/api/companies/?limit=1000');
    ...
    try {
      ...
      for (const form of companyForms) {
        ...
        if (initialValues && initialValues.id && index === 0) {
          res = await axios.put(
            `https://pplus-6xcn.onrender.com/api/companies/update/${initialValues.id}`,
            payload
          );
        } else {
          res = await axios.post(
            'https://pplus-6xcn.onrender.com/api/companies/create',
            payload
          );
        }
