...
import { competitiveSentimentData } from '@/utils/competitiveIntelligenceData';
...
export function CompetitiveSentimentPage() {
  ...
  const slightlyPositiveColor = "#10b981"; // Green
  const moderatelyPositiveColor = "#059669"; // Darker green
  const positiveColor2 = "#047857"; // Even darker green
  const stronglyPositiveColor = "#065f46"; // Darkest green

  return (
    <div className="space-y-6 animate-fade-in">
      <h2 className="text-2xl font-bold">Competitive Sentiment Intelligence - Holdings</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Top - Competitive Media Sentiment Score */}
        <DataCard title="Top - Competitive Media Sentiment" variant="glass" icon={<TrendingUp size={24} />}>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={competitiveSentimentData.sentimentScore}
                layout="vertical"
              ...
              >
                ...
                <Bar dataKey="value" name="Score">
                  {competitiveSentimentData.sentimentScore.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
        ...
        <DataCard title="Top - Competitive Media Sentiment Distribution Matrix" variant="glass" icon={<BarChart2 size={24} />}>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={competitiveSentimentData.sentimentDistributionMatrix}
                layout="vertical"
                margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
              >
                <CartesianGrid strokeDasharray="3 3" />
                ...
                />
                <Tooltip />
                <Bar dataKey="slightlyPositive" stackId="a" fill={slightlyPositiveColor} name="Slightly Positive" />
                <Bar dataKey="moderatelyPositive" stackId="a" fill={moderatelyPositiveColor} name="Moderately Positive" />
                <Bar dataKey="positive" stackId="a" fill={positiveColor2} name="Positive" />
                <Bar dataKey="stronglyPositive" stackId="a" fill={stronglyPositiveColor} name="Strongly Positive" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </DataCard>
        ...
        <DataCard title="Top - Competitive Media Sentiment Frequency" variant="glass" icon={<BarChart2 size={24} />}>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={competitiveSentimentData.sentimentFrequency}
                layout="vertical"
                margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
              >
                <CartesianGrid strokeDasharray="3 3" />
