...
export function CoverageRegionPage() {
  ...
  const fetchSocialData = useCallback(async () => {
    ...
    try {
      ...
      const url = `https://pplus-oez4.onrender.com/api/v1/report/social-stats-online-coverage?${params.toString()}`;
  ...
  const getPlatformIcon = (platform: string) => {
    switch (platform) {
      case 'x':
        return <Twitter size={24} className="text-white" />;
      case 'facebook':
        return <Facebook size={24} className="text-white" />;
      case 'instagram':
        return <Instagram size={24} className="text-white" />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in relative">
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center space-y-4 border border-gray-100">
            ...
            <div className="text-center">
              <p className="text-lg font-semibold text-gray-800">
                Loading coverage data for {companyName}
              </p>
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 rounded-2xl p-8 text-white relative overflow-hiddenn">
          ...
          <div className="flex items-center gap-4">
            ...
            {socialData && (
              <div className="text-right">
                ...
                <div className="text-2xl font-bold text-white">{socialData.summary?.countries_covered || 0}</div>
                <div className="text-sm text-green-200">{socialData.online_country_coverage?.total_coverage_records || 0} total records</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div className="xl:col-span-3 space-y-8">
          <div>
            <h2 className="text-xl font-bold text-gray-800 mb-4">Social Media Metrics</h2>
              ...
              {socialPlatforms.length === 0 ? (
                ...
                socialPlatforms.map((stat, index) => (
                  <Card key={index} className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                    <div className={`bg-gradient-to-br ${getPlatformColor(stat.platform)} p-4`}>
                      ...
                      <div className="space-y-4">
                        {stat.totalPosts !== '0' && (
                          <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">{stat.totalPosts}</p>
                          </div>
                        )}
                        {stat.followers !== '0' && (
                          <div>
                        ...
                        {stat.following !== '0' && (
                          <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">{stat.following}</p>
                          </div>
                        )}
                        {stat.likes && (
                          <div>
                            <p className="text-sm text-gray-500">Page Likes</p>
                            <p className="text-2xl font-bold text-gray-900">{parseInt(stat.likes).toLocaleString()}</p>
                          </div>
                        )}
                        {stat.avgLikes && (
                          <div>
                            <p className="text-sm text-gray-500">Avg Likes per Post</p>
                            <p className="text-xl font-bold text-gray-900">{parseInt(stat.avgLikes).toLocaleString()}</p>
                          </div>
                        )}
                        {stat.avgComments && (
                          <div>
                            <p className="text-sm text-gray-500">Avg Comments per Post</p>
                            <p className="text-xl font-bold text-gray-900">{parseInt(stat.avgComments).toLocaleString()}</p>
                          </div>
                        )}
                      </div>
                    </CardContent>
                  ...
                  <div className="bg-blue-50 rounded-lg p-3 mb-4">
                    <p className="text-sm text-blue-800 font-medium">
                      {socialData?.online_country_coverage?.unique_countries || 0} {
                        socialData?.online_country_coverage?.unique_countries === 1 ? 'Country' : 'Countries'
                      }
                    </p>
                  </div>
                  {regionalCoverage.map((region, index) => (
                    <div key={index} className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 hover:border-gray-300 transition-colors">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-green-600">{region.percentage}%</p>
                        </div>
                      </div>
                    </div>
                  ))}
