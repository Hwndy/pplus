...
export function SwotMentionsPage() {
  ...
  const fetchSwotData = async (page: number = 1) => {
    ...
    try {
      ...
      const endpoint = role === 'Supervisor'
        ? 'https://pplus-07cr.onrender.com/api/swot-analysis/supervisor-mentions'
        : role === 'Analyst'
        ? 'https://pplus-07cr.onrender.com/api/swot-analysis/my-analysis'
        : 'https://pplus-07cr.onrender.com/api/swot-analysis';
  ...
  const handleDelete = async (swot: SwotAnalysis) => {
    ...
    try {
      const response = await fetch(`https://pplus-07cr.onrender.com/api/swot-analysis/delete/${swot.id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
      });
