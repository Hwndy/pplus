...
export function SwotMentionsPage() {
  ...
  const fetchSwotData = async (page: number = 1) => {
    ...
    try {
      ...
      const endpoint = role === 'Supervisor'
        ? 'https://pplus-e31a.onrender.com/api/swot-analysis/supervisor-mentions'
        : role === 'Analyst'
        ? 'https://pplus-e31a.onrender.com/api/swot-analysis/my-analysis'
        : 'https://pplus-e31a.onrender.com/api/swot-analysis';
  ...
  const handleDelete = async (swot: SwotAnalysis) => {
    ...
    try {
      const response = await fetch(`https://pplus-e31a.onrender.com/api/swot-analysis/delete/${swot.id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
      });
