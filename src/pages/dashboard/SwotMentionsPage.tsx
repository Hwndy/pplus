...
export function SwotMentionsPage() {
  ...
  const fetchSwotData = useCallback(async (page: number = 1, limit: number = 10) => {
    ...
    try {
      let endpoint = 'https://pplus-7q0p.onrender.com/api/v1/swot-analysis';

      if (isSupervisor) {
        endpoint = 'https://pplus-7q0p.onrender.com/api/v1/swot-analysis/supervisor-mentions';
      } else if (isAnalyst) {
        endpoint = 'https://pplus-7q0p.onrender.com/api/v1/swot-analysis/my-analysis';
      }
  ...
  const handleDelete = async (swot: SwotAnalysis) => {
    ...
    try {
      const response = await fetch(`https://pplus-7q0p.onrender.com/api/v1/swot-analysis/delete/${swot.id}`, {
        method: 'PUT',
