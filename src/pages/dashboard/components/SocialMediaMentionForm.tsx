...
export function SocialMediaMentionForm({ mode, initialData, onSuccess }: SocialMediaMentionFormProps) {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        const response = await fetch('https://pplus-gl9c.onrender.com/api/v1/companies', {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = mode === 'create'
      ? 'https://pplus-gl9c.onrender.com/api/v1/social-media-mentions/create'
      : `https://pplus-gl9c.onrender.com/api/v1/social-media-mentions/update/${initialData?.id}`;
