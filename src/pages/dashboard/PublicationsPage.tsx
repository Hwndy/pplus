...
const PublicationsPage: React.FC = () => {
  ...
  const fetchPublications = async (page = 1, search = '') => {
    try {
      ...
      const response = await fetch(
        `https://pplus-tk49.onrender.com/api/publications?page=${page}&limit=${pageSize}&search=${search}`
      );
  ...
  const handleDelete = async (id: number) => {
    try {
      setDeletingId(id);
      const response = await fetch(`https://pplus-tk49.onrender.com/api/publications/delete/${id}`, {
        method: 'PUT',
      });
      const result = await response.json();
      if (result.success) {
  ...
  const handleSavePublication = async (publicationData: Publication) => {
    try {
      ...
      const url = editingPublication
        ? `https://pplus-tk49.onrender.com/api/publications/update/${editingPublication.id}`
        : `https://pplus-tk49.onrender.com/api/publications/create`;
