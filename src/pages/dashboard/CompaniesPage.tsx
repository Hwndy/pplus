...
const CompaniesPage = () => {
  ...
  const fetchCompanies = async () => {
    setLoading(true);
    try {
      const res = await axios.get('https://pplus-oez4.onrender.com/api/v1/companies/', {
        params: apiParams
      });
  ...
  const openFormDialog = async (company: any = null, viewMode = false) => {
    if (company && !viewMode) {
      try {
        const res = await axios.get(`https://pplus-oez4.onrender.com/api/v1/companies/${company.id}`);
  ...
  const handleDelete = async (id: string, companyName: string) => {
    try {
      await axios.put(`https://pplus-oez4.onrender.com/api/v1/companies/delete/${id}`);
