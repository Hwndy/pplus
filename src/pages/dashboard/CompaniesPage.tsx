...
const CompaniesPage = () => {
  ...
  const fetchCompanies = async () => {
    setLoading(true);
    try {
      const res = await axios.get('https://pplus-ec37.onrender.com/api/companies/', {
        params: apiParams
      });
  ...
  const openFormDialog = async (company: any = null, viewMode = false) => {
    if (company && !viewMode) {
      try {
        const res = await axios.get(`https://pplus-ec37.onrender.com/api/companies/${company.id}`);
  ...
  const handleDelete = async (id: string, companyName: string) => {
    try {
      await axios.put(`https://pplus-ec37.onrender.com/api/companies/delete/${id}`);
