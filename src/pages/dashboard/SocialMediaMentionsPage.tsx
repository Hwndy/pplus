...
export default function SocialMediaMentionsPage() {
  ...
  const fetchMentions = async (page = 1, limit = 10) => {
    ...
    setError(null);
    try {
      const response = await fetch(`https://pplus-2myh.onrender.com/api/social-media-mentions?page=${page}&limit=${limit}`, {
        headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`https://pplus-2myh.onrender.com/api/social-media-mentions/delete/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`,
  ...
  const handleUpdateStatus = async (id: number, status: 'Approved' | 'Rejected') => {
    try {
      const response = await fetch(`https://pplus-2myh.onrender.com/api/social-media-mentions/${id}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`,
