...
export default function OutcomeInsightsPage() {
  ...
  const fetchOutcomeInsights = async (page = 1, limit = 10) => {
    ...
    try {
      ...
      const endpoint = user.role.name === 'Supervisor'
        ? `https://backend-e79r.onrender.com/api/outcome-insights/supervisor-mentions?page=${page}&limit=${limit}`
        : user.role.name === 'Analyst'
        ? `https://backend-e79r.onrender.com/api/outcome-insights/my-insights?page=${page}&limit=${limit}`
        : `https://backend-e79r.onrender.com/api/outcome-insights?page=${page}&limit=${limit}`;
  ...
  const handleDelete = async (outcome: OutcomeInsight) => {
    if (window.confirm(`Are you sure you want to delete the outcome insight for ${outcome.company.company_name}?`)) {
      try {
        const response = await fetch(`https://backend-e79r.onrender.com/api/outcome-insights/delete/${outcome.id}`, {
          method: 'PUT',
          headers: getAuthHeaders(),
        });
        const result = await response.json();
  ...
  const handleCreateOrUpdate = async (formData: any) => {
    ...
    try {
      const response = await fetch(`https://backend-e79r.onrender.com${url}`, {
        method,
        headers: getAuthHeaders(),
